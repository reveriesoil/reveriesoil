...
基于 AI 的视觉小说生成器。输入一段提示词，AI 自动完成故事剧本创作、角色立绘生成、场景背景绘制与语音合成，打包成可直接游玩的视觉小说。
...
从 [Releases](https://github.com/reveriesoil/reveriesoil/releases) 页面下载 ReverieSoil-x.x.x.apk。

安装前需在系统设置中允许「未知来源」应用安装，然后直接打开 APK 文件即可。

---

### 方式三：Docker 部署（服务器 / 本地开发）（服务器 / 本地开发）
