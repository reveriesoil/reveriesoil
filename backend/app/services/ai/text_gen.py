...
async def generate_director_vision(
    ...
    system = f"""你是视觉小说的艺术总监（导演），负责整体视觉和声音风格设计。
...
IMAGE_PROMPTS_SCHEMA = {
    ...
    "description": "执行导演：输出人物/场景/CG 绘图提示词",
    "parameters": {
        "type": "object",
        "properties": {
            "character_prompts": {
        ...
        "required": ["character_prompts", "background_prompts", "cg_prompts"],
    },
}
...
- 人物 prompt：外貌、服装、妆造、绘图风格、构图要求；指定 vertical 9:16 portrait composition、full body 或 upper body、面向相机、绿幕背景以便后续抠像
...
- is_animatable=true 的 CG 应适合动态化（如飘落的樱花、闪烁的光效）"""
    ...
    return await _call_tool(
        _client(api_key, endpoint), model, system, user,
        IMAGE_PROMPTS_SCHEMA, temperature=0.5, max_tokens=8192,
    )
