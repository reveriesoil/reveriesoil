...
_SIZE_BACKGROUND = "2560x1440"   # 16:9 横版背景
        ...
        try:
            async with _get_semaphore():
                ...
                async with httpx.AsyncClient(timeout=120) as client:
                    ...
                    if item.get("url"):
                        img_resp = await client.get(item["url"], timeout=60)
...
async def generate_portrait(
    ...
    prompt = (
        f"{character_appearance}，{expr_desc}，"
        f"全身站立，单人居中，2D 动漫插画，赛璐璐上色，"
        f"纯绿色背景 #00FF00，chroma key green，单一纯色，"
        f"竖版 9:16 构图"
    )
